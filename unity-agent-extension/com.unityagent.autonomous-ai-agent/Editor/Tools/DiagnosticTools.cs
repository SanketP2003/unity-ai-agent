...
namespace AutonomousUnityAgent.Editor.Tools
{
    ...
    public class GetConsoleLogsTool : IBridgeTool
    {
        ...
        public static List<LogEntryData> GetLogs(int count, Func<LogEntryData, bool> filter = null)
        {
            EnsureHooked();
            var results = new List<LogEntryData>();
            lock (LogBuffer)
            {
        ...
        public BridgeMessage Execute(BridgeMessage request)
        {
            ...
            var filtered = GetLogs(count, entry =>
                filterType == "All" || string.Equals(entry.type, filterType, StringComparison.OrdinalIgnoreCase));
